...
#include "minishell.h" 
 ...
 void   rewrite_line(t_input_info *terminal, char *line, t_bool erase_mode, t_bool duplicate_string)
 {
    if (erase_mode)
        free(terminal->line);
    if (duplicate_string)
...
void    retrieve_older_command(t_input_info *terminal)
{
    if (terminal->current_history)
    {
        if (!terminal->current_history->prev && !terminal->h_saved_line)
        {
            ...
            rewrite_line(terminal, terminal->current_history->content, false, true);
        }
        else
        {
            if (terminal->current_history->next)
                terminal->current_history = terminal->current_history->next;
            rewrite_line(terminal, terminal->current_history->content, true, true);
        }
    }
}
